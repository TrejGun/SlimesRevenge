...
{
    /// <summary>
    /// Passive rear enemy stays put (FixedRng does not wander), so the slime can stand on the
    /// first corpse, dump one unit, devour both blood units, strike the next enemy, then
    /// collect the puddle back to capacity. Enemies are softened to one hit; MaxHitPoints stay 5
    /// so the first corpse's HP-decay outlives the dump/devour/strike sequence.
    /// </summary>
    public class TwoEnemiesDumpDevourCollectTests : DuelFixture
    {
        [Test]
        public void DumpDevourAttackCollect_ReturnsToFull()
        {
            ...
            var rear = Passive(new Vector2Int(6, 0));
